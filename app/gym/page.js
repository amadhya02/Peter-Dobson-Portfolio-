...
export default function GymPage() {
  return <PageShell eyebrow={gym.hero.eyebrow} title={gym.hero.title} outline={gym.hero.outline} intro={gym.hero.intro}>
    <section className="gym-feature subpage-section"><div className="gym-feature-logo"><img src={media.gymLogo} alt={`${media.gymLogoAlt} logo`} /></div><div><p className="eyebrow light"><span /> {gym.feature.eyebrow}</p><h2>{gym.feature.headingLine1}<br />{gym.feature.headingLine2}</h2><p>{gym.feature.text}</p><div className="feature-list">{gym.feature.list.map((item) => <span key={item}>{item}</span>)}</div><div className="gym-actions"><a className="button button-light" href="https://truenorthgym.nl/en">{gym.feature.primaryCta} <span>↗</span></a><a className="text-link light-link" href="https://truenorthgym.nl/en/book">{gym.feature.secondaryCta}</a></div></div></section>
