...
export default function PricingPage() {
  return <><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(faqJsonLd).replace(/</g, '\\u003c') }} /><PageShell eyebrow={pricing.hero.eyebrow} title={pricing.hero.title} outline={pricing.hero.outline} intro={pricing.hero.intro} variant="pricing" cue="From €54 / session">
    <section className="price-grid subpage-section">{pricing.plans.map((plan, i) => <article className={`price-card reveal-scale${plan.featured ? ' price-featured' : ''}`} key={plan.title}>{plan.featured && <span className="popular-badge">Most popular</span>}<span className="plan-number">0{i + 1}</span><TrainingIcon name={['calendar', 'commitment', 'monthly', 'single'][i]} className="price-icon" /><p className="card-label">{plan.tag}</p><h2>{plan.title}</h2><div className="price"><strong>{plan.price}</strong><span>{plan.suffix}</span></div><p>{plan.text}</p><p className="recommended-for">{plan.recommendedFor}</p><ul className="plan-includes">{plan.includes.map((item) => <li key={item}>{item}</li>)}</ul><Link href="/intake">{pricing.planCta} <span>→</span></Link></article>)}</section>
